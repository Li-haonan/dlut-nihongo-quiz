...
describe('compact progress code', () => {
  ...
  it('rejects removed full and legacy sync-code formats', () => {
    expect(() => parseProgressCode('DLUTSYNC:old-full-backup')).toThrow('前缀')
    expect(() => parseProgressCode('DLUTPROG:1:AAAA')).toThrow('仅支持当前')
    expect(() => parseProgressCode('DLUTPROG:2:AAAA')).toThrow('仅支持当前')
  })
})
